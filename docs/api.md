...
> **Cache:** resultado em memória por 5 min. Use `refresh=true` para forçar recomputação.
...
| `symbol` | `string` | não | Ativo analisado. Default: `BTCUSDT`. |
| `interval` | `IntervalEnum` | não | Timeframe. Default: `1h`. |
| `refresh` | `'true'` | não | Ignora o cache e força nova computação. |
...
> **Cache:** resultado em memória por 5 min por combinação `(symbol, interval, windowCount)`. Use `refresh=true` para forçar recomputação.
...
| `refresh` | `'true'` | — | Ignora o cache e força nova computação |
