...
export type ResolveHistoricalTradeParams = Pick<
  HistoricalTrade,
  'action' | 'entry' | 'stop' | 'target' | 'patterns' | 'confidence' | 'entryTime'
> & {
  futureCandles: Candle[];
};

export function isActionableSuggestion(action: TradeActionEnum): boolean {
  return action === TradeActionEnum.Buy || action === TradeActionEnum.Sell;
}

export function resolveHistoricalTrade(params: ResolveHistoricalTradeParams): HistoricalTrade {
...
export function buildHistoricalSimulationMetrics(trades: HistoricalTrade[]): HistoricalSimulationResult['metrics'] {
...
export function buildHistoricalPatternStats(trades: HistoricalTrade[]): HistoricalPatternStat[] {
...
export function buildHistoricalSimulationResult(
  params: { symbol: string; interval: IntervalEnum },
  trades: HistoricalTrade[],
): HistoricalSimulationResult {
  const patternStats = buildHistoricalPatternStats(trades);

  return {
    symbol: params.symbol,
    interval: params.interval,
    trades,
    metrics: buildHistoricalSimulationMetrics(trades),
    patternStats,
