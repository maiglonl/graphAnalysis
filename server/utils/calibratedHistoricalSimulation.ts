...
import { buildScoreCalibration, getSuggestionScoreAdjustment, type ScoreCalibrationResult } from '#shared/utils/scoreCalibration';
...
export function runCalibratedHistoricalSimulation(
  ...
  const calibrated = buildSimulationResult(params, calibratedTrades);
...
function averageReturn(trades: HistoricalTrade[]): number {
  ...
  return trades.reduce((sum, trade) => sum + tradeReturn(trade), 0) / trades.length;
}
