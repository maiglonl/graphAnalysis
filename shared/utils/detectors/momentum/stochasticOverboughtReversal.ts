...
export class StochasticOverboughtReversalDetector extends MomentumPatternDetector {
  detect(ctx: ScanContext) {
    ...
    if (
      ...
      prevK < EXTRA_THRESHOLDS.stochasticOverbought ||
      prevK < prevD ||
      currK >= currD
    ) return [];
    return this.buildSignal(ctx, PatternIdEnum.StochasticOverboughtReversal, PatternDirectionEnum.Bearish, EXTRA_CONFIDENCE.stochasticOverboughtReversal);
  }
}
