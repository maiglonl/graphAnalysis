...
export class StochasticOverboughtReversalDetector extends PatternDetector {
  detect(ctx: ScanContext): PatternSignal[] {
    ...
    if (
      ...
      previousK < EXTRA_THRESHOLDS.stochasticOverbought ||
      previousK < previousD ||
      currentK >= currentD
    ) return [];
    ...
    return [{
      ...
      confidence: EXTRA_CONFIDENCE.stochasticOverboughtReversal,
      price: current.close,
      entry,
      stop: entry + risk,
