...
export class OnNeckDetector extends CandlePatternDetector {
  readonly id = PatternIdEnum.OnNeck;
  readonly direction = PatternDirectionEnum.Bearish;
  readonly baseConfidence = EXTRA_CONFIDENCE.onNeck;
  ...
  protected override match(candles: Candle[], index: number, ctx: ScanContext): MatchResult | null {
    ...
    if (Math.abs(curr.close - prevBodyLow) > tolerance) return null;
