...
export class LongLeggedDojiDetector extends CandlePatternDetector {
  override readonly id = PatternIdEnum.LongLeggedDoji;
  override readonly direction = PatternDirectionEnum.Neutral;
  override readonly baseConfidence = EXTRA_CONFIDENCE.longLeggedDoji;
  ...
  protected override match(candles: Candle[], index: number, _ctx: ScanContext) {
    ...
    if (parts.upperShadow / parts.range < EXTRA_THRESHOLDS.longLeggedDojiMinShadowPct) return null;
    if (parts.lowerShadow / parts.range < EXTRA_THRESHOLDS.longLeggedDojiMinShadowPct) return null;
